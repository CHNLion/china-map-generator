...
**从 v2.2.1 版本开始，项目已内置思源黑体字体文件，无需额外配置即可正常显示中文！**

## 问题描述

在Render或其他Linux服务器上部署后，生成的地图中所有中文都显示为方框（□□□）。

**原因**：Linux服务器默认没有安装中文字体，且matplotlib无法找到可用的中文字体。

**解决方案**：项目已内置 `app/static/fonts/SourceHanSansSC-Regular.otf`（思源黑体），代码会自动优先使用该字体文件。
...
1. **内置字体文件**：
   - 路径：`app/static/fonts/SourceHanSansSC-Regular.otf`
   - 字体：Adobe 思源黑体 SC Regular（15.8MB）
   - 随项目代码一起部署，无需系统安装
