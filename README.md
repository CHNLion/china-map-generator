...
### 🎨 高级颜色选择器
- **Pickr 专业颜色选择器**：渐变色板、色相滑块、预设颜色
- **实时预览与自动关闭**：拖动选色圈选择颜色，释放鼠标后自动应用并关闭
- **预设颜色板**：16种常用颜色，点击即选
- **吸管取色工具**：从屏幕任意位置取色（Chrome/Edge 95+ 支持）
...
使用 Pickr 专业颜色选择器，提供 Windows 风格的直观操作：

**选色方式**：
1. **渐变色板**：拖动选色圈精确选择颜色，释放鼠标后自动应用并关闭
2. **色相滑块**：快速切换色相
3. **预设色块**：16种常用颜色，点击即选并关闭
4. **吸管工具**：点击吸管图标，从屏幕任意位置取色
   - 支持浏览器：Chrome 95+、Edge 95+、Opera 81+
   - 取色成功后自动应用并关闭

**输入方式**：
- **HEX 输入框**：直接输入十六进制颜色值（如 #FF0000）

**交互特点**：
- 完全模仿 Windows 原生颜色选择器的使用习惯
- 无需手动点击"保存"按钮
- 按 `Escape` 键可随时关闭调色板
- **实时预览**：选择时即时预览效果
...
**三种专业样式**：
- **分段式（黑白交替）**：经典地图样式，黑白方块交替，每段都有数值标注（默认推荐）
- **刻度线式**：简洁风格，主线加刻度，适合现代地图
- **双行交替式**：双层黑白交替，视觉效果更丰富

**智能比例计算**：
- 自动根据地图大小选择合适的比例尺长度
- 比例尺占地图宽度约35%，视觉效果适中
- 自动选择整数量程（5、10、20、50、100、200、300、500、1000、1500、2000 km）
- 确保比例精确匹配实际地理距离（1:1比例）
...
6. 选择字体大小：12像素
7. 点击"生成地图"
备注：比例尺会自动计算合适的量程（如500km或1000km），确保比例准确
```
...
- ✨ 新增吸管取色工具
  - 支持从屏幕任意位置取色
  - 使用浏览器原生 EyeDropper API
  - 取色成功后自动应用并关闭调色板
- 🎨 优化颜色选择器交互体验
  - 实现 Windows 风格的颜色选择器
  - 拖动选色圈释放鼠标后自动应用
  - 点击预设色块立即应用并关闭
  - 移除冗余的 Save/Cancel 按钮和 HEXA 切换按钮
- 🎨 优化吸管按钮布局和样式
  - 使用专业的 SVG 吸管图标
  - 优化按钮位置，与输入框对齐
  - 添加悬停和点击动画效果
- 🎨 使用说明改为默认折叠状态，界面更简洁
- 🌐 添加网站图标和 PWA 支持
  - 添加多尺寸 favicon（.ico, 16x16, 32x32, 48x48, 192x192, 512x512）
